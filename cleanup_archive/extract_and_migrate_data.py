#!/usr/bin/env python3
"""
Script para extraer y migrar datos del backup de manera directa
"""
                ...
                for line in ann_lines:
                    if line.strip():
                        ...
                        if len(parts) >= 13:
                            ...
                            VALUES ('{ann_id}', '{customer_name}', '{phone_number}', '{guide_number}', {is_active.lower() == 't'}, {is_processed.lower() == 't'}, '{announced_at}', {f"'{processed_at}'" if processed_at != '\\N' else 'NULL'}, {f"'{customer_id}'" if customer_id != '\\N' else 'NULL'}, {f"'{package_id}'" if package_id != '\\N' else 'NULL'}, '{created_at}', '{updated_at}', '{tracking_code}')
                            ON CONFLICT (id) DO NOTHING;
                            """
