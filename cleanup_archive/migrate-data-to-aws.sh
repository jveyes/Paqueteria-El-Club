#!/bin/bash
# ========================================
# PAQUETES EL CLUB v3.1 - Migración de Datos a AWS
# ========================================
...
# Verificar conexión a la base de datos AWS
log_info "Verificando conexión a base de datos AWS..."
...
TABLES=("users" "customers" "packages" "announcements" "package_status_history")

for table in "${TABLES[@]}"; do
    COUNT=$(PGPASSWORD="$AWS_DB_PASSWORD" psql -h "$AWS_DB_HOST" -p "$AWS_DB_PORT" -U "$AWS_DB_USER" -d "$AWS_DB_NAME" -t -c "SELECT COUNT(*) FROM $table;" | xargs)
    if [ "$COUNT" != "" ]; then
